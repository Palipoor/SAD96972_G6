...
public class DBManager {
	...
	private void setManagersToDefault() {
		String query = String.format("update manager_manager as m set company_rial_credit= %s, company_dollar_cent_credit=%s, company_euro_cent_credit=%s;",
	...
	private void setCustomersToDefault() {
		String query = String.format("update customer_customer as c set rial_credit= %s, dollar_cent_credit=%s, euro_cent_credit=%s;", String.valueOf(Defaults.DEFAULT_CUSTOMER_RIAL_CREDIT),
				String.valueOf(Defaults.ZERO), String.valueOf(Defaults.ZERO));
		try {
			Statement stmt = conn.createStatement();
			stmt.executeUpdate(query);
	...
	private void deleteManagers() {
		String customers_delete = "delete from manager_manager WHERE genuser_ptr_id=(select user_ptr_id from main_genuser where user_ptr_id=(select id from auth_user where username like 'TEST_%'));";
	...
	public void setManagerToDefault(String username) {
		String query = String.format("update manager_manager as m set company_rial_credit= %s, company_dollar_cent_credit=%s, company_euro_cent_credit=%s  where m.genuser_ptr_id=" +
	...
	public void setEmployeeToDefault(String username) {
		String query = String.format("update employee_employee as e set current_salary= %s where m.genuser_ptr_id=" +
