...
def add_transaction_types(apps, schema_editor):
    CustomTransactionType = apps.get_model('customer', 'CustomTransactionType')
    for name, label, description in Transactions.transaction_types:
        customtransactiontype = CustomTransactionType(type=name, label=label, description=description)
        customtransactiontype.save()
    # ContentType = apps.get_model('contenttypes', 'ContentType')
    # new_ct = ContentType.objects.get_for_model(CustomTransactionType)
    # CustomTransactionType.objects.filter(polymorphic_ctype__isnull=True).update(polymorphic_ctype=new_ct)


def remove_transaction_types(apps, schema_editor):
    CustomTransactionType = apps.get_model('customer', 'CustomTransactionType')
    types = CustomTransactionType.objects.all()
    types.delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(add_transaction_types, remove_transaction_types),
