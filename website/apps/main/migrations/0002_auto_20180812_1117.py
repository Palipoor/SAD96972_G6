...
def add_charge(apps, schema_editor):
    ...
    customer = Customer.objects.get(username='customer', email='customer@customer.com')
    for i in range(0, 5):
        charge = Charge(dest_user=customer, dest_wallet="0", amount=i*10)
...
def add_exchange(apps, schema_editor):
    ...
    customer = Customer.objects.get(username='customer', email='customer@customer.com')
    exchange = Exchange(source_user=customer, source_wallet="0", amount=100000, dest_wallet="1")
    customer.rial_credit -= 100000
    customer.dollar_cent_credit += 1000
    exchange.save()
    exchange = Exchange(source_user=customer, source_wallet="0", amount=100000, dest_wallet="2")
    customer.rial_credit -= 100000
...
def add_foreignTrans(apps, schema_editor):
    ...
    customer = Customer.objects.get(username='customer', email='customer@customer.com')
    foreignTrans = ForeignTrans(source_user=customer, source_wallet="0", amount=30000, account_number='12312412', bank_name='mammadbagherbank')
    customer.rial_credit -= 30000 + 3000 * foreignTrans.profitRate
    foreignTrans.save()
    foreignTrans = ForeignTrans(source_user=customer, source_wallet="1", amount=100, account_number='12312412', bank_name='mammadbagherbank')
    ...
    customer.dollar_cent_credit -= 100*(1*foreignTrans.profitRate)
    foreignTrans.save()
    foreignTrans = ForeignTrans(source_user=customer, source_wallet="2", amount=100, account_number='12312412', bank_name='mammadbagherbank')
    ...
    customer.save()
    ContentType = apps.get_model('contenttypes', 'ContentType')
    new_ct = ContentType.objects.get_for_model(ForeignTrans)
    ForeignTrans.objects.filter(polymorphic_ctype__isnull=True).update(polymorphic_ctype=new_ct)
...
def remove_foreignTrans(apps, schema_editor):
    ...
    customer = Customer.objects.get(username='customer', email='customer@customer.com')
    Customer = apps.get_model('customer', 'Customer')
    foreignTrans = ForeignTrans.objects.get(source_user=customer)
    foreignTrans.delete()
